...
-- 2) Let's extract the main actor & their gender, and store them in columns "actor" and "gender".
	ALTER TABLE credits
	ADD actor TEXT, gender VARCHAR(1)

	UPDATE credits
	SET actor = SUBSTRING(cast, CHARINDEX('"name"', cast)+9, CHARINDEX('"order"', cast)-CHARINDEX('"name"', cast)-12)
	...
	UPDATE credits
	SET gender = SUBSTRING(cast, CHARINDEX('"gender"', cast)+10, 1)
	WHERE "cast" LIKE '%"character"%'
